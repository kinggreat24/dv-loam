...
class ProjectionParams {
 public:
  ...
  static std::unique_ptr<ProjectionParams> HDL_64_EQUAL(const int hsize, const int vsize);

  //北科天绘激光雷达
  /**
   * @brief      Default parameters for 16 beam RFans16 北科天绘１６线激光雷达参数
   *
   * @return     A pointer to parameters
   */
  static std::unique_ptr<ProjectionParams> RFANS_16();
  
  /**
   * @brief      Default parameters for 16 beam CFans32 北科天绘32线FOV激光雷达参数
   *
   * @return     A pointer to parameters
   */
  static std::unique_ptr<ProjectionParams> CFANS_32();
