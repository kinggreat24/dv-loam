...
std::unique_ptr<ProjectionParams> ProjectionParams::HDL_64_EQUAL(const int hsize, const int vsize)
{
  auto params = ProjectionParams();
  params.SetSpan(SpanParams(-180_deg, 180_deg, hsize),
                 SpanParams::Direction::HORIZONTAL);
  params.SetSpan(SpanParams(2.0_deg, -24.0_deg, vsize),
                 SpanParams::Direction::VERTICAL);
  params.FillCosSin();
  if (!params.valid())
  {
...
//北科天绘１６线激光雷达
std::unique_ptr<ProjectionParams> ProjectionParams::RFANS_16()
{
  auto params = ProjectionParams();
  params.SetSpan(SpanParams(-180_deg, 180_deg, 870),
                 SpanParams::Direction::HORIZONTAL);
  params.SetSpan(SpanParams(15.0_deg, -15.0_deg, 16),
                 SpanParams::Direction::VERTICAL);
  params.FillCosSin();
  if (!params.valid())
  {
...
//北科天绘窄视角32线激光雷达参数
std::unique_ptr<ProjectionParams> ProjectionParams::CFANS_32()
{
  auto params = ProjectionParams();
  params.SetSpan(SpanParams(-60_deg, 60_deg, 600),
                 SpanParams::Direction::HORIZONTAL);
  
  //32线的数据分为８组，每组４条ring，范围为[8.4 degree ~ -10.2 degree]
  SpanParams span_bin1( 8.4_deg,    6.9_deg,  4);
  SpanParams span_bin2( 6.0_deg,    4.5_deg,  4);
  SpanParams span_bin3( 3.6_deg,    2.1_deg,  4);
  SpanParams span_bin4( 1.2_deg,   -0.3_deg,  4);
  SpanParams span_bin5(-1.2_deg,   -2.7_deg,  4);
  SpanParams span_bin6(-3.6_deg,   -5.1_deg,  4);
  SpanParams span_bin7(-6.0_deg,   -7.5_deg,  4);
  SpanParams span_bin8(-8.5_deg,   -10.2_deg, 4);
  
  vector<SpanParams> spans = {{span_bin1, span_bin2,span_bin3,span_bin4,span_bin5,span_bin6,span_bin7,span_bin8}};
  params.SetSpan(spans, SpanParams::Direction::VERTICAL);
  params.FillCosSin();
  if (!params.valid())
  {
