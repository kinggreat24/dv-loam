...
typename pcl::PointCloud<pcl::PointXYZI>::Ptr Cloud::ToPcl(int cluster_id) const
{
  using pcl::PointXYZI;
  using PclCloud = pcl::PointCloud<PointXYZI>;
  PclCloud pcl_cloud;
...
typename pcl::PointCloud<pcl::PointXYZRGB>::Ptr Cloud::ToPcl(Eigen::Vector3d rgb) const
{
  ...
  using PclCloud = pcl::PointCloud<PointXYZRGB>;
  PclCloud pcl_cloud;
  for (const auto &point : _points)
  {
    PointXYZRGB pcl_point;
    pcl_point.x = point.x();
    pcl_point.y = point.y();
    pcl_point.z = point.z();
    pcl_point.r = rgb.x();
