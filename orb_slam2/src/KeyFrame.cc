...
{
    //相机模型
    mpPinholeCamera         = F.mpPinholeCamera;

    //激光点云特征
    mpCornerPointsSharp     = F.mpCornerPointsSharp;
    mpCornerPointsLessSharp = F.mpCornerPointsLessSharp;
    mpSurfPointsFlat        = F.mpSurfPointsFlat;
    mpSurfPointsLessFlat    = F.mpSurfPointsLessFlat;

    mpGroundPoints          = F.mpGroundPoints;

    mpMagLidarPointCloud    = F.mpMagLidarPointCloud;
    mnTrackLevels           = F.mnTrackLevels;
    mvImgPyramid            = F.mvImgPyramid;
...
float KeyFrame::get_visible_ratio (const Sophus::SE3& T_cur_ref)
{
    ...
    for (auto iter=mpMagLidarPointCloud->begin(); iter!=mpMagLidarPointCloud->end(); ++iter) {
        
        Eigen::Vector3d xyz_ref (iter->x, iter->y, iter->z);
        Eigen::Vector3d xyz_cur = T_cur_ref*xyz_ref;
        if(xyz_cur.z() < 0)
            continue;

        Eigen::Vector2d uv_cur  = mpPinholeCamera->world2cam(xyz_cur) * scale;

        if (!mpPinholeCamera->isInFrame(uv_cur.cast<int>(), border, current_level))
            continue;
        
        visible_points++;
    }

    return static_cast<float> (visible_points) / static_cast<float> (mpMagLidarPointCloud->size());
}
